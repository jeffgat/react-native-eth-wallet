...
type TokenProps = {
  ...
  handleTokenPress: any;
};

const Token = ({ item, handleTokenPress }: TokenProps) => {
  return (
    <TouchableOpacity
    ...
    >
      <View className="flex-row items-center">
        ...
        <View>
          <View className="flex-row">
            <BaseText className="font-semibold mr-1">{item.abbr}</BaseText>
            <BaseText
      ...
      <View className="items-end justify-end">
        ...
        <BaseText className="opacity-60" ellipsizeMode="tail" numberOfLines={1}>
          ~ ${formatNumberWithCommas(item.usdBalance)} USD
        </BaseText>
      </View>
    </TouchableOpacity>
  );
